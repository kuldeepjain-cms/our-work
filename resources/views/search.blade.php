...
<head>
  <META NAME="ROBOTS" CONTENT="NOINDEX, NOFOLLOW">
    ...
    <link href="{{ URL::asset('template/frontend/css/bootstrap-multiselect.css') }}" rel="stylesheet">
    <link href="{{ URL::asset('template/frontend/css/bootstrap-select.min.css') }}" rel="stylesheet">
    <link rel="stylesheet" type="text/css" href="{{ URL::asset('template/frontend/css_new/cubeportfolio1.min.css') }}">
    <link href="{{ URL::asset('template/frontend/css/font-awesome.min.css') }}" rel="stylesheet">
    <link href="{{ URL::asset('template/frontend/css/search.css') }}" rel="stylesheet">
</head>
<body>
  ...
  <!-- scrollToTop -->
  <!-- ================ -->
<div class="wrapper container wrap-container">
    <form action="{{ url('searchresult') }}" id="search" method="post" style="display: none;">
      <input type="hidden" name="_token" value="{{ csrf_token() }}">
      <div class="col-lg-3 col-md-3 col-sm-3 col-xs-12 selectfield">
        <label>Category:</label>
        <select name="category" class="selectpicker">
          <option value="all">All</option>
          <?php foreach ($categories as $category): ?>
            <option value="{{ $category->category_id }}" @if(isset($search->category) && $search->category == $category->category_id) selected='selected' @endif>{{ $category->category_name }}</option>
          <?php endforeach; ?>
        </select>
      </div>
      <div class="col-lg-3 col-md-3 col-sm-3 col-xs-12 selectfield">
      ...
      <select name="technology" class="selectpicker">
        ...
        <?php foreach ($technologies as $technology): ?>
          <option value="{{ $technology->technology_name }}" @if(isset($search->technology) && $search->technology == $technology->technology_name) selected='selected' @endif>{{ $technology->technology_name }}</option>
        <?php endforeach; ?>
      </select>
      </div>
      <div class="col-lg-3 col-md-3 col-sm-3 col-xs-12 selectfield">
      <label>Tags:</label>
      <select name="tags[]" class="multiple" multiple>
        <?php foreach ($technologies as $technology): ?>
          <option value="{{ $technology->technology_name }}" @if(isset($search->tags) && in_array($technology->technology_name, $search->tags)) selected='selected' @endif>{{ $technology->technology_name }}</option>
        <?php endforeach; ?>
      </select>
      ...
      <div class="col-lg-3 col-md-3 col-sm-3 col-xs-12 selectfield">
      <label>Server:</label>
      <select name="web_server" class="selectpicker">
        <option value="">Please Select server</option>
        <?php foreach ($web_servers as $web_server): ?>
          <option value="{{ $web_server->server_id }}" @if(isset($search->web_server) && $search->web_server == $web_server->server_id) selected='selected' @endif>{{ $web_server->server_name }}</option>
        <?php endforeach; ?>
      </select>
      </div>
      <div class="col-lg-3 col-md-3 col-sm-3 col-xs-12 selectfield">
      ...
      <select name="theme" class="selectpicker">
        ...
        <?php foreach ($themes as $theme): ?>
          <option value="{{ $theme->theme_id }}" @if(isset($search->theme) && $search->theme == $theme->theme_id) selected='selected' @endif>{{ $theme->theme_name }}</option>
        <?php endforeach; ?>
      </select>
      </div>
      <div class="col-lg-3 col-md-3 col-sm-3 col-xs-12 selectfield">
      <label>Packages:</label>
      <select name="package[]" class="multiple" multiple>
        <?php foreach ($packages as $package): ?>
          <option value="{{ $package->package_id }}" @if(isset($search->package) && in_array($package->package_id, $search->package)) selected='selected' @endif>{{ $package->package_name }}</option>
        <?php endforeach; ?>
      </select>
      <div class="clearfix"></div>
      </div>
      ...
      <select name="payment[]" class="multiple" multiple>
        <?php foreach ($payments as $payment): ?>
          <option value="{{ $payment->payment_id }}" @if(isset($search->payment) && in_array($payment->payment_id, $search->payment)) selected='selected' @endif>{{ $payment->payment_name }}</option>
        <?php endforeach; ?>
      </select>
      <div class="clearfix"></div>
      </div>
      ...
      <input id="reset" type="reset" name="reset" value="Reset Search" />
      </div>
    </form>
    <div class="clearfix"></div>
    <div id="grid-container" class="cbp-l-grid-gallery cbp cbp-loading cbp-caption-overlayBottomPush">
      <div class="cbp-wrapper">
        ...
        @foreach ($portfolios as $portfolio)
          <div class="cbp-item <?php echo strtolower(str_replace(' ', '', $portfolio->technology)); ?>">
            <div class="cbp-caption">
                ...
                <div class="cbp-caption-activeWrap">
                    <div class="cbp-l-caption-alignCenter">
                      <div class="cbp-l-caption-title" style="display:block;height: 30px;">{{$portfolio->title}}</div>
            <div class="cbp-l-caption-desc" style="display:block;height: 30px;">{{$portfolio->technology}}</div>
            ...
            <div class="clearfix"></div>
        @endforeach        
        @elseif(isset($portfolios) && count($portfolios) == 0)
          <div>
            <label>No Portfolio found</label>
          </div>
        @endif
      </div>
    </div> 
...
<!-- footer start (Add "light" class to #footer in order to enable light footer) -->
      ...
      <section class="footerbg">
        ...
        href="tel:+079 4890 7823">079 4890 7823</a></span>
            ...
            <a title="Top of page" class="btn-scroll-to-top" href="#top" style="display: none;">
    ...
    <script type="text/javascript" src="{{ URL::asset('template/frontend/js/bootstrap-multiselect.js') }}"></script>
    <script type="text/javascript" src="{{ URL::asset('template/frontend/js/bootstrap-select.min.js') }}"></script>
    <script type="text/javascript" src="{{ URL::asset('template/frontend/js_new/jquery.cubeportfolio1.min.js') }}"></script>
    <script type="text/javascript" src="{{ URL::asset('template/frontend/js_new/main1.js') }}"></script>
    ...
    $(document).ready(function(){
      ...
      $('#search select.selectpicker').selectpicker({
        size: 3
      });
      $('#reset').click(function(){
        setTimeout(function() {
          $('#search select.selectpicker').selectpicker("refresh");
          $('#search select.multiple').multiselect("refresh");
        });
      });
    });
    </script>
